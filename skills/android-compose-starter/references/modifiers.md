...
// Do: provide a meaningful contentDescription for informational images
Image(
    painter = painterResource(id = R.drawable.icon),
    contentDescription = "User avatar"
)

// Do: use null for purely decorative images — tells screen reader to skip it
Image(
    painter = painterResource(id = R.drawable.background_pattern),
    contentDescription = null  // Correct for decorative images
)

// Don't: omit contentDescription parameter entirely for meaningful images
// (unlike null, omitting may result in the resource name being announced)
Image(painter = painterResource(id = R.drawable.icon)) // Missing contentDescription
```

> See [accessibility.md](accessibility.md) for full guidance on when to use `null` vs a string description.
...
### Not Accepting a Modifier Parameter

Modifier chains like `Modifier.padding(16.dp).background(Color.Blue)` are cheap to create and completely fine inside a composable. The real anti-pattern is not accepting a `modifier` parameter — this prevents callers from customising layout or adding accessibility semantics.

```kotlin
// Don't: callers cannot customise layout or add semantics
@Composable
fun BadModifier() {
    Box(Modifier.padding(16.dp).background(Color.Blue)) { }
}

// Do: accept modifier as first parameter, apply caller modifier before internal ones
@Composable
